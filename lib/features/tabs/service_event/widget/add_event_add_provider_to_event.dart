...
class AddEventAddProviderSheet extends StatefulWidget {
  final AddServiceEventBloc? addServiceEventBloc;

  const AddEventAddProviderSheet({
    Key? key,
    this.addServiceEventBloc,
  }) : super(key: key);

  @override
  _AddEventAddProviderSheetState createState() =>
      _AddEventAddProviderSheetState();
}

class _AddEventAddProviderSheetState extends State<AddEventAddProviderSheet> {
  @override
                    ...
                    children: [
                      ...
                      VehicleTextField(
                        textEditingController:
                            widget.addServiceEventBloc?.txtName,
                      ...
                      VehicleTextField(
                        textEditingController:
                            widget.addServiceEventBloc?.txtEmail,
                      ...
                      VehicleTextField(
                        textEditingController:
                            widget.addServiceEventBloc?.txtPhoneNum,
                        ...
                        inputFormatters: [
                          MaskTextInputFormatter(
                            mask: '(###) ###-####',
                            initialText:
                                widget.addServiceEventBloc?.txtPhoneNum?.text,
                          ),
                        ],
                      ),
                      SizedBox(height: 20),
                      TypeAheadAddressField(
                        controller: widget.addServiceEventBloc?.address,
                        onSelected: (suggestion) {
                          setState(() {
                            widget.addServiceEventBloc?.getProviderAddresses(
                                context: context, val: suggestion);
                            widget.addServiceEventBloc?.address?.text =
                                suggestion;
                          });
                        },
                        suggestionsCallback: (pattern) async {
                          if (pattern.length >= 3) {
                            try {
                              await widget.addServiceEventBloc
                                  ?.getProviderAddress(context: context);
                              return widget.addServiceEventBloc?.addressList;
                            } catch (error) {
                              throw Exception(
